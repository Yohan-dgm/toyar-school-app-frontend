    ...
    >({
      ...
      transformResponse: (response: any) => {
        ...
        if (response?.success === false && response?.errors) {
          ...
          throw new Error(`Validation failed: ${response.message || 'Check required parameters'}`);
        }
        ...
        return {
          data: [],
          metadata: { current_page: 1, total: 0, per_page: 20, last_page: 1, from: 0, to: 0 },
          unread_count: 0,
        };
      },
    }),
    ...
    >({
      query: ({ filters = {} }) => {
        const requestBody = {
          ...
          include_read: filters.include_read !== undefined ? filters.include_read : true,
          include_archived: filters.include_archived !== undefined ? filters.include_archived : false,
        };
      ...
      transformResponse: (response: any) => {
        ...
        if (response?.success === false && response?.errors) {
          ...
          throw new Error(`Stats validation failed: ${response.message || 'Check required parameters'}`);
        }
