...
import {
  ...
  clearFilters,
  toggleLike,
} from "../../../state-store/slices/school-life/class-posts-slice";
...
// Dummy class ID for development - replace with actual student's class ID from backend
const DUMMY_CLASS_ID = 5;

const ClassTabWithAPI = ({ userCategory, isConnected, filters }) => {
  ...
  } = useSelector((state) => state.classPosts);

  // API hooks
  const [getClassPosts, { isLoading: apiLoading }] =
    useLazyGetClassPostsQuery();
  ...
  // Set dummy class ID on component mount
  useEffect(() => {
    if (!currentClassId) {
      dispatch(setCurrentClassId(DUMMY_CLASS_ID));
    }
  ...
  const fetchPosts = useCallback(
    async (pageNum = 1, isRefresh = false, currentFilters = {}) => {
      ...
      try {
        ...
        const response = await getClassPosts({
        ...
        if (response.success) {
          ...
          dispatch(setError(response.message || "Failed to fetch class posts"));
        }
      } catch (error) {
        ...
        if (__DEV__) {
          Alert.alert(
            "API Not Available",
            `Using dummy class ID: ${currentClassId}. Please implement the backend API endpoints.`,
            [{ text: "OK" }]
          );
        }
      } finally {
    ...
    [getClassPosts, dispatch, storeFilters, currentClassId]
  );

  // Initial load when class ID is set
  useEffect(() => {
    if (currentClassId) {
      fetchPosts(1, false);
    }
  ...
  }, [filters, dispatch, fetchPosts, currentClassId]);
    ...
    return (
      <View style={styles.postContainer}>
        ...
        <View style={styles.classIndicator}>
          <Text style={styles.classText}>Class {currentClassId}</Text>
        </View>
